...
export default function Home({ onStart, onIdioms, onSynonymsAntonyms, onHomonymsHomophones, onEnglishVocab, onGrammar, onNarration, onVoiceChange, onVocabTest, onGeometry, onGeography, onIndiaMap }) {
  ...
  const handleModuleClick = (module) => {
    ...
    if (selectedSubject === 'english' && selectedTopic === 'Grammar') {
      ...
      if (module.title.includes('Narration')) {
        onNarration();
      ...
      if (module.title.includes('One Word') || module.title.includes('Vocabulary')) {
        onStart({ start: 0, limit: 250 });
      } else if (module.title.includes('Idioms')) {
        onIdioms({ start: 0, limit: 250 });
      } else if (module.title.includes('Synonyms')) {
        onSynonymsAntonyms({ start: 0, limit: 250 });
      } else if (module.title.includes('Homonyms')) {
        onHomonymsHomophones({ start: 0, limit: 250 });
      } else if (module.title.includes('Geometry')) {
        onGeometry();
      } else if (module.title.includes('Test')) {
        onVocabTest();
      }
    } else {
      ...
      if (module.title.includes('Vocabulary') || module.title.includes('One Word')) {
        ...
        setShowPYQ(true);
      } else if (module.title.includes('Geography')) {
        // Handle geography subtopics
        if (selectedSubject === 'gs' && selectedTopic === 'Geography') {
          // This will be handled by subtopic selection
          return;
        } else {
      ...
      } else if (module.title.includes('India Map')) {
        onIndiaMap();
                ...
                selectedTopic ? (
                  ...
                  selectedSubject === 'english' && selectedTopic === 'Vocabulary' ? (
                    ...
                    <>
                      ...
                      <div style={{ ...card, borderColor: "#e67e22" }} onClick={() => onIdioms({ start: 0, limit: 250 })}>
                      ...
                      <div style={{ ...card, borderColor: "#9c27b0" }} onClick={() => onSynonymsAntonyms({ start: 0, limit: 250 })}>
                      ...
                      <div style={{ ...card, borderColor: "#3b82f6" }} onClick={() => onHomonymsHomophones({ start: 0, limit: 250 })}>
                  ...
                  ) : selectedSubject === 'english' && selectedTopic === 'Grammar' ? (
                    // Show grammar topics
                    <>
                      <div style={{ ...card, borderColor: "#7b1fa2" }} onClick={() => handleModuleClick({ title: "📝 Narration" })}>
                        <h3>📝 Narration</h3>
                        <p>Learn Direct and Indirect Speech with examples</p>
                        <span style={{ ...activeTag, backgroundColor: "#7b1fa2" }}>Available</span>
                      </div>
                      
                      <div style={{ ...card, borderColor: "#ff9800" }} onClick={() => handleModuleClick({ title: "🗣️ Voice Change" })}>
                        <h3>🗣️ Voice Change</h3>
                        <p>Practice Active and Passive Voice transformation</p>
                    ...
                    <>
                      ...
                      <div style={{ ...card, borderColor: "#ff9800" }} onClick={() => alert('World Geography coming soon!')}>
                        ...
                        <p>Countries, continents, and world geographical features</p>
                        <span style={{ ...activeTag, backgroundColor: "#ff9800" }}>Coming Soon</span>
                      </div>
                    </>
                  ) : (
                    // Show default modules for other subtopics
