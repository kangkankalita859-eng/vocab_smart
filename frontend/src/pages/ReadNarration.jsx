...
import { fetchNarration } from "../services/narrationService";
import SessionNav from "../components/SessionNav";
import MobileSidebar from "../components/MobileSidebar";
import useMobile from "../hooks/useMobile";

export default function ReadNarration({ config, onUpdateConfig, onGoHome }) {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        setLoading(true);
        const response = await fetchNarration(0, 228); // Load all questions
        setQuestions(response.data);
        setCurrentQuestionIndex(null); // Start with no question selected
        setSelectedOption(null);
        setShowAnswer(false);
      } catch (error) {
        console.error("Error loading narration questions:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div style={loadingContainer}>
        <div style={loadingContent}>
          ...
          <p>Loading narration questions...</p>
        </div>
      </div>
    );
  }
      ...
      <div style={content}>
        ...
        {isMobile && (
          <div style={mobileNav}>
            ...
            <span style={mobileTitle}>Narration Practice</span>
            <button
              style={homeBtn}
              onClick={onGoHome}
            >
...
const questionsListContainer = {
  ...
  marginBottom: "20px",
  // Remove max-height and overflow-y to enable full page scrolling
};
...
const questionHeader = {
  ...
  justifyContent: "space-between",
  alignItems: "center",
  marginBottom: "18px" // Reduced from 25px
};
...
const sentenceContainer = {
  ...
  border: "1px solid #e9ecef", // Reduced from 2px
  marginBottom: "18px" // Reduced from 25px
};
...
const explanationContainer = {
  ...
  border: "1px solid #d4edda" // Reduced from 2px
};
